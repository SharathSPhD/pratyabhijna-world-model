...
H5b is a documented honest negative result: against a 120B unconditioned
LLM that is near-ceiling on English-script domains, PWM-conditioned
generation scores $\mu_{\text{PWM}} = 0.788$ versus
...
A separate Phase~8 ablation measures H5b, an out-of-distribution check on
the live text-only camatkāra scorer rather than on the imagination-level
metric used for H5a.
We collected $n=30$ paired samples ($10$ per domain: English pop, Carnatic
kṛti, and Kannada film lyric) generated by the PWM-conditioned cascade
(VimarsaBridgeV2 with WMReasoningTrace prefill, Section~\ref{sec:phase7})
and by the unconditioned bare \texttt{nemotron-3-super:120b} LLM served via
Ollama.
...
construction); see Section~\ref{sec:gen-summary} for its definition.
...
full $R_{\text{camatk}}$ (VFE, Hopfield novelty, empowerment); the live
H5b ablation isolates \emph{text-surface} quality only.
We conclude that the world model's measurable contribution is in the
internal creative process --- sphurattā events, vimarśa state, long-horizon
VFE minimisation, and the latency contradiction resolved in
Section~\ref{sec:phase7} --- rather than in improving text-surface camatkāra
scores over a sufficiently powerful unconditioned LLM baseline on
English-script domains.
This is an important boundary condition on world-model augmentation of
near-ceiling LLMs that we believe is best disclosed openly, and it
motivates the multilingual, lower-resource evaluation we outline in
...
Camatkāra evaluation uses the deterministic \path{score_camatk_text}
scorer in \path{pwm/eval/camatk_eval.py}; the full statistical pipeline
...
family-wise correction~\cite{holm1979simple}) is implemented in
\path{pwm/eval/statistics.py}.
