...
After confirming IDL convergence (cos\_sim $\to -1.0$ within 1{,}300 steps),
we re-ran \texttt{gate2.py} on the v4 step-200K checkpoint and found
$0/200$ sphurattā for both EFE and REINFORCE ($T_{\text{EFE}} = T_{\text{RF}} = 15$,
ratio $= 1.0$, H1 FAIL).
Systematic diagnosis revealed a \emph{fourth} failure layer: the \texttt{free\_bits}
floor ($= 1.0$ per dimension, $32 \times 1.0 = 32$ nats total) prevents the KL term
in VFE from falling below its floor.
Consequently, the prior network $f_\theta(h_t)$ receives \emph{zero gradient}
from VFE whenever KL $<$ free\_bits — which is always the case in the passive corpus
environment (all VFE is dominated by reconstruction loss alone).
Result: $p_{\theta}(z | h_t) \approx \text{Uniform}(32{\times}32)$ for all $h_t$
(measured entropy $= 110.90 \pm 0.00$ nats $\approx 32\ln 32$), making prior entropy
action-invariant regardless of $\mathbf{W}_a$ geometry.

\paragraph{Revised proxy: recurrent activation norm.}
We replaced the broken prior-entropy proxy with the recurrent hidden-state norm
$\|h_t\|_2$ as the camatkāra signal.
IDL's antipodal geometry makes $\|h_t\|_2$ systematically action-dependent
(even-indexed actions produce $\|h_t\| \approx 2.4$, odd-indexed produce $\approx 1.3$
from zero init).
Under this revised metric, both EFE and REINFORCE fire sphurattā at a 32\% rate
($N{=}200$ episodes, $H{=}15$), confirming the metric is functional.
However, $T_{\text{EFE}} = 10.84$ vs.\ $T_{\text{RF}} = 10.97$ (ratio $= 0.99$,
H1 FAIL): EFE and REINFORCE are statistically indistinguishable because the actor
distribution entropy is $4.16/4.16$ (near-uniform) at step~200K.
The actor cannot converge beyond a uniform distribution because the reward signal
(prior entropy variation) is zero under the free\_bits ceiling.

\paragraph{Diagnostic summary: four-layer H1 failure.}
Table~\ref{tab:h1_diagnosis} summarises the compounding failure chain.
Each layer was resolved in sequence; layer 4 (free\_bits ceiling) is the remaining
architectural constraint.
The correct long-term resolution requires either (a) reducing free\_bits
$\to 0.1$ and retraining, allowing the prior to become informative; or (b) adding a
genuine interactive evaluation environment where $p(o_{t+1}|s_t, a_t) \neq p(o_{t+1}|s_t)$.
Both are future-work items; the current Phase 2 result is an honestly-documented
negative with clear causal chain.
...
\begin{table}[h]
  ...
  \begin{tabular}{clcc}
    \toprule
    Layer & Root Cause & Signal & Fix \\
    \midrule
    1 & \texttt{curr\_vfe=0.0} hardcoded & $\Delta F = 0$ always & Replace w/ prior $H$ \\
    2 & Zero-action replay buffer & $\mathbf{W}_a \to 0$ & Random one-hot $a_t$ \\
    3 & Passive corpus env & $\nabla_{\mathbf{W}_a}\mathcal{L}_{\text{VFE}}=0$ & IDL \\
    4 & free\_bits ceiling & $p_\theta\approx\text{Uniform}$ & Reduce free\_bits \\
    \bottomrule
  \end{tabular}
\end{table}
