...
\paragraph{Zero-reward diagnostic.}
The initial 300K run revealed a plumbing bug: imagination rollouts passed
\texttt{curr\_vfe=0.0} to the camatkāra reward, making $\Delta F = 0$ for all
300K steps. The actor trained as a max-entropy policy (svātantrya prior only),
which is a valid exploration warm-start but not sufficient for H1 evaluation.

\paragraph{Fix and re-run.}
We replaced the static \texttt{curr\_vfe=0.0} with the WM's imagined prior
entropy $H(p_{\text{prior}}(z_t|h_t))$ as the intrinsic reward signal:
%
\begin{equation}
R_{\text{camatk}} \propto \Delta H_{\text{prior}} = \max\bigl(H(p^{(t-1)}) - H(p^{(t)}), 0\bigr)
\end{equation}
%
This signal is fully action-dependent (different actions $\to$ different $h_t$
$\to$ different prior distributions $\to$ different entropy) and aligns exactly
with EFE's epistemic objective.

% TODO: Fill after Phase 2 re-run completes. H1 result:
