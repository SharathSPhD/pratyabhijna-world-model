...
(threshold: 1.10, PASS); Phase~4 (sleep consolidation) launched automatically.
Phases~4--6 (H3--H9) are in progress; results will be updated upon gate passage.
...
Aparā uses a GRU recurrent core; Parā uses an S4 structured state-space
model~\cite{gu2022efficiently}, providing $O(T \log T)$ parallel training
and $O(T)$ recurrent inference for long-horizon prediction.
In Phases 1--4, only the Aparā level (L0) is active; the full three-level
hierarchy is evaluated in Phase~6 (H7).
