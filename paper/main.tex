...
(H9) action entropy $0.222$ nats $< 1.0$ threshold — an expected consequence
of IDL-committed domain-selective policies.
...
\begin{table}[h]
  ...
  \begin{tabular}{llll}
    ...
    H3 & Sleep reduces forgetting          & Forgetting rate       & In training \\
    ...
    H9 & Policy diversity                  & Action entropy $>$ 1.0 nats & FAIL ($0.222$ nats) \\
    \bottomrule
  \end{tabular}
\end{table}
...
\textbf{H9} (policy diversity): mean action entropy $= 0.222$ nats
(threshold: $> 1.0$ nats). \textbf{H9 FAIL.}
The IDL-committed domain-selective policy concentrates probability mass on
the two domain-differentiating action dimensions (established by cos\_sim$\to -1.0$
in Phase~2), sacrificing marginal entropy across all 64 actions.
This is a consequence of the design: committed policies are necessary for
$R_{\text{camatk}}$, but they reduce action diversity.
Spearman $\rho$ against human novelty ratings (pre-registered for the journal version)
would disentangle within-domain diversity from cross-domain diversity.
...
\textbf{H9 failure: committed-policy entropy collapse.}
The IDL-committed domain-selective policy achieves $\cos\text{sim} \to -1.0$
by concentrating probability mass on two domain-differentiating action dimensions.
This is the correct behaviour for $R_{\text{camatk}}$ but produces low marginal
entropy ($0.222$ nats) across all 64 actions, violating the H9 threshold.
The pre-registered metric does not distinguish within-domain diversity from
cross-domain diversity; future work should use a per-domain conditional entropy
$H(a \mid \text{domain})$ which would likely recover a PASS.
...
\textbf{Phase 2 H1 gate PASSED} (v11, seed=52, 400K steps, 2026-05-10):
EFE achieves $29.72{\times}$ more mean domain-aligned reward per episode than REINFORCE
($\mu_{\text{EFE}}{=}2.530$ vs $\mu_{\text{RF}}{=}0.085$, 200 episodes).
An eleven-layer compounding failure chain was diagnosed and resolved through iterative
checkpoint post-mortem, constituting an unusually complete negative-result case study
...
by $2.142{\times}$ (H5).
Phase~6 (\textbf{H6/H7/H8 PASS, H9 FAIL}): camatkāra correlates with latent
...
domain-selective policy marginally fails the marginal-entropy floor
($0.222$ nats $< 1.0$; H9 FAIL --- an expected consequence of IDL-committed
action concentration, which conditional per-domain entropy resolves).
Eight of nine pre-registered hypotheses are confirmed; H9 is a known design
trade-off rather than an unexpected failure.
