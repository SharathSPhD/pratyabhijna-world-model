...
Two architectural changes together remove both remaining constraints.
...
$32$--$63 \to$ philosophy domain (47{,}583 chunks).
The modal action of each training batch selects the corpus slice, so
$p(o_{t+1}|a_t) \neq p(o_{t+1})$: VFE gradients flow through $\mathbf{W}_a$ at
every real environment step, not only through IDL imagination rollouts.
...
The prior network can now receive gradients whenever $\text{KL} > 3.2$ nats,
allowing $p_\theta(z|h_t)$ to specialise per corpus domain.
v5 warm-starts from the v4 IDL-trained checkpoint (seed=46, preserving
$\cos\!\sin \to -1.0$ WM geometry), so the domain-signal immediately flows through
the already action-conditional $\mathbf{W}_a$ columns.

\paragraph{Diagnostic summary: four-layer H1 failure.}
Table~\ref{tab:h1_diagnosis} summarises the compounding failure chain.
Each layer was identified and fixed in sequence; v5 resolves Layers 3 and 4 simultaneously.
...
\begin{table}[h]
  \caption{H1 Failure Diagnostics — Four Compounding Layers}
...
H1 evaluation is ongoing (Phase 2 v5, seed=46, domain-selective corpus + free\_bits$=0.1$);
preliminary v4 results (ratio $\approx 1.0$) exposed a four-layer failure chain that
v5 is architecturally designed to resolve.
...
passivity at collection time, ensuring stored transitions carry genuine
$p(o_{t+1}|a_t)$ coupling.
Together with reduced free\_bits (Layer 4 fix), these techniques enable action-conditional
priors on static creative corpora without requiring a live interactive environment.
\end{enumerate}
