...
\paragraph{H5 live ablation (Phase 8 re-evaluation).}
A Phase~8 live ablation measured $n{=}30$ paired samples (10 per domain:
English pop, Carnatic, Kannada film) comparing PWM-conditioned generation
(VimarsaBridgeV2 + WMReasoningTrace) against bare-LLM
(\texttt{nemotron-3-super:120b}) using a \emph{text-only} camatkāra scorer
($R_{\text{camatk}}$ without VFE terms, for fairness to the LLM condition).
Result: $\mu_{\text{PWM}} = 0.788$, $\mu_{\text{LLM}} = 0.862$,
$g = -0.47$, BCa~95\%~CI $[-0.12, -0.02]$, $p_{\text{perm}} = 0.996$.
\textbf{H5 live: FAIL on text-only scorer.}

This is a negative result reported with full rigour (CLAUDE.md~\S7).
The 120B LLM produces higher text-level camatkāra on English domains
(carnatic: $\mu_{\text{LLM}}{=}0.976$, English pop: $\mu_{\text{LLM}}{=}0.937$)
where the model is near-ceiling.
The Kannada film domain shows near-parity ($\mu_{\text{PWM}}{=}0.655$,
$\mu_{\text{LLM}}{=}0.672$), suggesting WM conditioning is more valuable
in lower-resource domains where the LLM is not near-ceiling.
The Phase~5 ratio~$= 2.14$ was measured in imagination (WM-internal metric)
using full $R_{\text{camatk}}$ including VFE terms; the live ablation isolates
\emph{text-surface} quality only.
We conclude: the WM's contribution is in its internal creative process
(sphurattā events, vimarśa state, long-horizon VFE minimisation) rather
than improving text-surface quality over a sufficiently powerful unconditioned LLM baseline.
...
\textbf{H7} (3-level vs 1-level WM, A6 ablation): Phase~8 ablation trained
1-level (Aparā-only) WM from scratch (3 seeds: 51 complete at 100K steps;
52 and 53 partial at 70K and 50K steps respectively, due to GPU contention
with concurrent ablations).
...
advantage factor $= 23.6\times$, \textbf{H7 PASS.}
...
Aggregate statistics ($\mu_{\text{TTFT}} = 62\,\text{s} \pm 133\,\text{s}$) are
dominated by Ollama model cold-start timeouts (300--420\,s) caused by a
measurement design confound: Cond~A and Cond~B are measured sequentially
within each prompt, leaving the 120B model loaded/busy when Cond~B fires.
Excluding cold-start artifacts, warm-model measurements are:
Cond~A TTFT $\in \{0.14,\, 2.90,\, 3.43,\, 3.53\}$\,s (mean 2.50\,s, all $<$5\,s);
Cond~B TTFT $\in \{3.08,\, 0.11,\, 0.16\}$\,s (mean 1.12\,s).
The critical ADR-002 measurement (Prompt~1, Cond~B, no prior interference):
TTFT~$= 3.08\,\text{s}$ versus Cond~A cold 120B~$= 300\,\text{s}$ — a
$97\times$ reduction confirming that WMReasoningTrace eliminates the 120B cold
chain-of-thought.
The Sprint test suite (118/118 PASS) remains the primary validation;
