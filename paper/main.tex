...
We pre-register nine hypotheses (H1--H9) and report completed results for
Phase~1--2: (H1) the EFE actor achieves $29.72{\times}$ more mean domain-aligned
reward per episode than REINFORCE ($\mu_{\text{EFE}}{=}2.530$ vs.\
$\mu_{\text{RF}}{=}0.085$, 200 episodes, $p{<}0.001$).
(H2) Hopfield Citta-store improves occlusion completion ratio to $1.307$
(threshold: 1.10, PASS);
(H3) sleep consolidation: negligible forgetting in both conditions (PASS);
(H4) 100\% sphurattā events satisfy $H(z_t){>}0.5$ nats (PASS);
(H5) mean reward ratio $2.142{\times}$ Phase~2 baseline (PASS).
Phase~6 (full ablations, H6--H9) is in training.
...
\begin{table}[h]
  ...
  \begin{tabular}{llll}
    ...
    H6 & Camatkāra: reward entropy         & Entropy $>$ 0.5 nats  & In training \\
    H7 & 3-level $>$ 1-level               & VFE proxy $< 0.85{\times}$ P3 & Pending \\
    H8 & Encoder stability                 & Norm $\in [1, 50]$    & Pending \\
    H9 & Policy diversity                  & Action entropy $>$ 1.0 nats & Pending \\
    \bottomrule
  \end{tabular}
\end{table}
...
$> 0.5$ nats over 500 episodes, confirming non-trivial camatkāra dynamics
(degenerate policies collapse to entropy $\approx 0$).
...
(KL divergence from uniform over $32{\times}32$ latents, measured over 200 imagination
episodes) $< 0.85\times$ Phase~3 VFE proxy, confirming that the full
Pancakṛtya system produces more structured latent dynamics than Hopfield alone.
16-step prediction MSE vs.\ 1-level Aparā-only is pre-registered for the journal version.
...
\textbf{H9 — Policy diversity:} mean action entropy $> 1.0$ nats over 200 episodes
(EFEActor), confirming a diverse rather than collapsed policy distribution;
Spearman $\rho$ against human novelty ratings is pre-registered for the journal version.
...
Phase 2 is complete (\textbf{H1 gate PASS}, 2026-05-10): the EFE actor achieves
$29.72{\times}$ more mean domain-aligned reward per episode than the REINFORCE baseline
($\mu_\text{EFE}{=}2.530$ vs $\mu_\text{RF}{=}0.085$, 200 episodes, seed=2025).
An eleven-layer compounding failure chain was diagnosed and resolved through iterative
post-mortem checkpoint analysis, constituting a reproducible diagnostic methodology
for active-inference RSSM training on static corpora.
Key contributions from Phase 2: (i) WM freeze at step~10{,}000 prevents gradient
starvation; (ii) domain-selective corpus routing + IDL produces action-conditional
hidden states on a passive corpus; (iii) consistency bonus + percentile clamping
breaks the cold-start deadlock; (iv) Layer~10 (fresh-sample PG) and Layer~11
(gate metric paradox) are newly characterised failure modes with concrete fixes;
(v) final checkpoint released as \texttt{final\_phase2\_v11\_seed52.pt}.
