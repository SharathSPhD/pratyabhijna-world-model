...
\paragraph{Phase 2 v10: action-consistency bonus + percentile clamping (training, 2026-05-10).}
...
For committed trajectories, $\sum_{t=1}^{H-1}\delta_t = (H-1)\cdot C = 24$, providing
strong differential advantage over alternating trajectories ($\sum_{t=1}^{H-1}\delta_t \approx 0$).
Advantage normalisation is replaced with DreamerV3 percentile clamping:
$\text{scale} = (Q_{0.95}-Q_{0.05}).\text{clamp}(\min{=}1.0)$, with advantage clipped to $[-1,1]$.
This prevents division by near-zero variance and is robust to outlier returns.
Training launched 2026-05-10, seed=51, warm-start from Phase~1 checkpoint.
Early diagnostic (step~200): critic entropy $2.45$ bits (down from $3.6$ at step~100),
confirming discriminative reward signal from the first batch. \emph{Full results pending.}

\paragraph{Diagnostic summary: nine-layer H1 failure chain.}
...
\begin{table}[h]
  \caption{H1 Failure Diagnostics — Nine Compounding Layers}
  ...
  \begin{tabular}{clcc}
    ...
    9 & Cold-start deadlock: $P(\text{committed}|\text{uniform},H{=}13)\!\approx\!0$ & $\mathcal{L}_\text{actor}\approx 0$ for 220K steps & Consistency bonus $+$ percentile clamping (v10) \\
    \bottomrule
  \end{tabular}
\end{table}
...
The nine-layer failure chain constitutes an unusually complete negative-result case study
...
Phase 2 has identified a nine-layer compounding failure chain through iterative
...
The most recent finding (Layer 9, cold-start deadlock) reveals a fundamental tension
in trajectory-level reward design: with a discrete action space of size $|\mathcal{A}|{=}64$
and horizon $H{=}13$, purely trajectory-level rewards have near-zero gradient under
random initialisation.
The resolution --- augmenting with a step-level Markov consistency bonus --- is a
general technique applicable to any RSSM trained with domain-commitment objectives.
Phase 2 v10 (consistency bonus + percentile clamping, seed=51) is currently training.
